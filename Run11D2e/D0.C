...
void D0(const int nEvents = 10000000, const int cen = 0, const int seed = 32553){
  ...
  float ptwtnorm = 20./charmspectrum->Integral(0.,20.);
  ...
  for (int Event = 0;Event<nEvents;Event++){
    ...
    float exwt = (ptRandom < ptex || charmspectrum->Eval(ptRandom)==0) ? 1. : funPL->Eval(ptRandom) / charmspectrum->Eval(ptRandom);
    //cout << exwt << endl;
    ptwt *= exwt;
  ...
  char outname[100];
  sprintf(outname,"D0levy%d.root",cen);
