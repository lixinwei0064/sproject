...
void Lc(const int nEvents = 10000000, const int cen = 0, const int seed = 32553){
  ...
  for (int Event = 0;Event<nEvents;Event++){
    ...
    float ptwt = Histo->Eval(ptRandom)*ptwtnorm;
    float exwt = (ptRandom < ptex || Histo->Eval(ptRandom)==0) ? 1. : funPL->Eval(ptRandom) / Histo->Eval(ptRandom);
    //cout << exwt << endl;
    ptwt *= exwt;
  ...
  char outname[100];
  sprintf(outname,"hLc_spectra_10_60_model%d.root",cen+1);
