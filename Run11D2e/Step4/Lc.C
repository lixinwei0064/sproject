...
void Lc(const int seed = 32553, const int nEvents = 10000){

  TRandom *myRandom = new TRandom(seed);

  TH2D *phiLc = new TH2D("phiLc","#phi distribution of #Lambda_{c};#phi;p_{T};count",18,0,6.283,80,0,8);
  ...
  TFile *phifile = new TFile(Form("output/e_Lc_1208.root"),"RECREATE");
  phiLc->Write();
  phie->Write();

  TF1 *func = new TF1("func","[1]*(1+2*[0]*TMath::Cos(2*x))",0,6.283);
  TH1D *v2D = new TH1D("v2D","distribution of v_{2} vs p_{T} of electron from #Lambda_{c} decay;p_{T};v_{2}",50,0.,5.);
  TH1D *phicount;
  for(int i=1;i<=50;i++){
  ...
  v2D->Write();
  phifile->Close();
  gPad->SaveAs("v2_vs_pT_e_Lc_1208.png");
}
