...
void plot_Ds() {
    ...
    fin->Close();*/
    TFile* fin = new TFile("data/Spectra_Sys_err.root");
    for(int icent=0; icent<ncent; icent++) {
        gSpectraUp[icent] = (TGraphErrors*)fin->Get(Form("ds_run14_%s",nameCent[icent]));
        gSpectraErr[icent] = (TGraphErrors*)fin->Get(Form("ds_run14_%s",nameCent[icent]));
        gSpectraSys[icent] = (TGraphErrors*)fin->Get(Form("sys_spectra_%s",nameCent[icent]));
        //gBand[icent] = (TGraphErrors*)fin->Get(Form("flevy_err_band_%s",nameCent[icent]));
        //fSpectra[icent] = (TF1*)fin->Get(Form("flevy_%s",nameCent[icent]));
    }
    fin->Close();
    ...
    for(int icent=0; icent<ncent; icent++) {
        ...
        for(int ipt=0; ipt<gBand[icent]->GetN(); ipt++) {
            ...
            //cout << sqrt((pow(err,2)+pow(sys,2))/old/old+pow(extErr,2)) << "\t" << sqrt((pow(err,2)+pow(sys,2))/old/old) << "\t" << pt << endl ;
